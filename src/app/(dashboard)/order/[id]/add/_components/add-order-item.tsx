...
const AddOrderItem = ({ id }: { id: string }) => {
  ...
  const { data: menus, isLoading: isLoadingMenu } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (result.error) {
        toast.error("Get menu failed", { description: result.error.message });
      }
  ...
  const { data: order } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (result.error) {
        toast.error("Get order failed", { description: result.error.message });
      }
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 w-full">
      <div className="space-y-4 lg:w-2/3">
        ...
        {!isLoadingMenu && !menus?.data?.length && (
